...
const options: IOption[] = [
    { value: 'TA-6A', label: 'TA-6A' },
    { value: 'GTCP85', label: 'GTCP85' }
]

const ApuAddForm: React.FC<EngineAddFormProps> = ({ setEngAddForm }) => {
    ...
    return (
        <div className={s.addEngineForm}>
            ...
            <Formik
                ...
                onSubmit={(values: IApu, actions) => {
                    if (selectedOption) values.type = selectedOption
                    dispatch(addApu(values))
                }}
            >
                {({ errors, touched }) => (
